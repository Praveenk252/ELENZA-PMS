(() => {
  ...
  function isPlannerOnlySession(user) {
    const loginId = String((user == null ? void 0 : user.login_id) || "").toLowerCase();
    const roleName = String((user == null ? void 0 : user.role_name) || "").toLowerCase();
    return loginId === "planner.user" || roleName === "production planner user";
  }
  function redirectPlannerPortal() {
    if (window.location.pathname.toLowerCase().indexOf("/planner-portal.html") >= 0) return;
    window.location.href = "/planner-portal.html";
  }
  async function restoreSession() {
    ...
    try {
      ...
      if (result.authenticated) {
        ...
        if (isPlannerOnlySession(state.session)) {
          redirectPlannerPortal();
          return;
        }
  ...
  function renderProductionQueueRow(row) {
    ...
    const panelQty = productionQty(row, "panel_qty");
    const boardQty = productionQty(row, "board_qty");
    return `
    <tr>
      ...
      <td>${escapeHtml(qtyLabel(panelQty))}</td>
      <td>${escapeHtml(qtyLabel(boardQty))}</td>
  ...
  function productionPlanningRow(row) {
    const rows = (((state.app || {}).planning || {}).rows || []);
    return rows.find((item) => String(item.order_id) === String(row.order_id)) || rows.find((item) => String(item.order_number || "").trim().toLowerCase() === String(row.order_number || "").trim().toLowerCase()) || null;
  }
  function productionQty(row, key) {
    const ownValue = row ? row[key] : "";
    if (String(ownValue || "").trim()) return ownValue;
    const planningRow = productionPlanningRow(row || {});
    return planningRow ? planningRow[key] : "";
  }
  ...
  function renderSharedPlanningTable(key, bodyRef, paginationRef, countRef, localSearch = "") {
    ...
    renderPagedTable({
      ...
      <tr class="planner-row planner-row-${escapeAttribute(row.planner_stage_key)}" data-shared-order-id="${row.order_id}">
        ...
        <td>${escapeHtml(qtyLabel(row.panel_qty))}</td>
        <td>${escapeHtml(qtyLabel(row.board_qty))}</td>
