<%@ WebHandler Language="C#" Class="PmsApiHandler" %>
...
public class PmsApiHandler : IHttpHandler, IRequiresSessionState
{
    ...
    private static readonly SampleUser[] SampleUsers = new[]
    {
        ...
        new SampleUser("Kiran Packing","packing.user","Machine User","Packing",true,"cc4ade1991db633a323c26e34e30be35","405dd54d6d1612af1782059f991c3c3ac0c970425c1e9f40ab69e393aab3534b",120000),
    ...
    private void HandleServeCompatScript(HttpContext context)
    {
        ...
        script = Regex.Replace(
            ...
            "state.session = result.user;\n        if (String((state.session && state.session.login_id) || \"\").toLowerCase() === \"planner.user\" || String((state.session && state.session.role_name) || \"\").toLowerCase() === \"production planner user\") { window.location.href = \"/planner-portal.html\"; return; }\n        await loadAppState(true);",
    ...
    {
        TraceAppState(context, "request started");
        try
        {
            using (var conn = OpenConnection(context))
            {
                var user = RequireLogin(context, conn);
                TraceAppState(context, "session user resolved: " + S(user, "login_id"));
                ...
                var payload = BuildAppState(conn, user, filters, context);
                TraceAppState(context, "payload built");
                WriteJson(context, payload);
                TraceAppState(context, "response written");
            }
        }
        catch (Exception ex)
        {
            TraceAppState(context, "caught exception: " + ex.GetType().FullName + " | " + ex.Message);
            throw;
        }
    }
    ...
    private void HandleUpdateDealerCustomerType(HttpContext context)
    {
        using (var conn = OpenConnection(context))
        {
            ...
            var dealersUpdated = Execute(conn, "UPDATE tbl_dealers SET customer_type_id = ?, customer_type_code = ?, updated_by = ?, updated_at = " + SqlDateLiteral(DateTime.Now) + " WHERE dealer_id = ?",
                I(customerType, "customer_type_id"), newType, I(user, "user_id"), dealerId);
            var ordersUpdated = Execute(conn, "UPDATE tbl_orders SET customer_type_id = ?, updated_by = ?, updated_at = " + SqlDateLiteral(DateTime.Now) + ", last_action = ? WHERE dealer_id = ?",
    ...
    private void HandleProductionBalanceSave(HttpContext context)
    {
        using (var conn = OpenConnection(context))
        {
            ...
            if (!IsPackingStationName(stationName))
                throw new ApiFailure(400, "Balance box qty save is only available for Packing.");
            if (S(user, "role_name") == "Machine User" && stationName != S(user, "station_name"))
                throw new ApiFailure(403, "Machine user can only update assigned station orders.");
            var balanceBoxQty = N(Value(context, "balance_box_qty"));
    ...
    private void HandlePlannerSave(HttpContext context)
    {
        using (var conn = OpenConnection(context))
        {
            ...
            Execute(conn, "UPDATE tbl_production_planner SET sla_date = " + SqlDateLiteral(slaDate) + ", urgency = ?, [priority] = ?, planner_remarks = ?, updated_by = ?, updated_at = " + SqlDateLiteral(now) + " WHERE planner_id = ?",
                NullIfEmpty(urgency),
            ...
            try
            {
                Audit(conn, I(user, "user_id"), "Production Planner", "Order", S(order, "order_number"), "Planner Updated", "", string.Join(" | ", new[] { string.IsNullOrWhiteSpace(urgency) ? "-" : urgency, string.IsNullOrWhiteSpace(priority) ? "-" : priority, string.IsNullOrWhiteSpace(plannerRemarks) ? "-" : plannerRemarks }), plannerRemarks, null);
            }
            catch
            {
            }
    ...
    private void HandlePackingBoxesSet(HttpContext context)
    {
        using (var conn = OpenConnection(context))
        {
            ...
            var stationName = Require(Value(context, "station_name"), "Station is required.");
            if (!IsPackingStationName(stationName))
                throw new ApiFailure(400, "Box qty entry is only available for Packing.");
            if (S(user, "role_name") == "Machine User" && stationName != S(user, "station_name"))
                throw new ApiFailure(403, "Machine user can only update assigned station orders.");
    ...
    private Dictionary<string, object> BuildAppState(OleDbConnection conn, Dictionary<string, object> user, Dictionary<string, string> filters, HttpContext context)
    {
        ...
        TraceAppState(context, "planning state built");
        ...
        TraceAppState(context, "history state built");
        ...
        var lifecycle = loadLifecycle
            ...
            : EmptyLifecycleState();
        TraceAppState(context, "lifecycle state built");
    ...
    private static void TraceAppState(HttpContext context, string message)
    {
        try
        {
            var path = context.Server.MapPath("~/App_Data/pms-appstate-trace.log");
            File.AppendAllText(path, DateTime.Now.ToString("s", CultureInfo.InvariantCulture) + " | " + message + Environment.NewLine);
        }
        catch
        {
            // Diagnostic logging must never change the application response.
        }
    }
    ...
    {
        return orders.Select(order => Obj(
            "order_id", I(order, "order_id"),
    ...
    private Dictionary<string, object> BuildPlanningRow(Dictionary<string, object> order, Dictionary<string, object> plannerRow, Dictionary<string, Dictionary<string, string>> statusLookup)
    {
        ...
        return Obj(
            ...
            "workflow_stage", Label(statusLookup, "WORKFLOW", S(order, "workflow_stage_code")),
            "procurement_status", Label(statusLookup, "PROCUREMENT", S(order, "procurement_status_code")),
            "dispatch_status", Label(statusLookup, "DISPATCH", S(order, "dispatch_status_code")),
            "visible_stations", ReadableVisibleStations(order),
    ...
    private string PlanningStageKey(Dictionary<string, object> order)
    {
        ...
        if (string.Equals(S(order, "workflow_stage_code"), "DISPATCH_READY", StringComparison.OrdinalIgnoreCase) || visibleStations.Contains("Dispatch")) return "packed";
    ...
    {
        var statusLookup = LoadStatusLookup(conn);
        if (S(user, "role_name") == "Machine User")
        {
            var stationHistory = QueryAll(conn, "SELECT TOP 120 h.order_id, h.acted_at, h.acted_by, h.station_id, h.action_code, h.remarks, h.from_station_id, h.to_station_id, h.previous_status_code AS old_status_code, h.new_status_code, u.full_name AS acted_by_name, s.machine_name AS station_name FROM (tbl_order_history AS h LEFT JOIN tbl_users AS u ON h.acted_by = u.user_id) LEFT JOIN tbl_machines AS s ON h.station_id = s.machine_id WHERE h.station_id = ? ORDER BY h.acted_at DESC, h.history_id DESC", I(user, "station_id"));
            var orderIds = stationHistory.Select(r => I(r, "order_id")).Where(v => v > 0).Distinct().ToList();
            var orderLookup = orderIds.Count == 0
                ? new Dictionary<int, Dictionary<string, object>>()
                : QueryAll(conn, "SELECT order_id, order_number, confirmation_date, packing_balance_box_qty FROM tbl_orders WHERE order_id IN (" + string.Join(",", orderIds) + ")")
                    .ToDictionary(r => I(r, "order_id"));
            var boxCountLookup = orderIds.Count == 0
                ? new Dictionary<int, int>()
                : QueryAll(conn, "SELECT order_id, COUNT(*) AS box_count FROM tbl_dispatch_boxes WHERE order_id IN (" + string.Join(",", orderIds) + ") GROUP BY order_id")
                    .ToDictionary(r => I(r, "order_id"), r => I(r, "box_count"));
            var machineRows = stationHistory.Select(row =>
            {
                Dictionary<string, object> order = null;
                orderLookup.TryGetValue(I(row, "order_id"), out order);
                return Obj(
                    "order_id", I(row, "order_id"),
                    "order_number", order == null ? "-" : S(order, "order_number"),
                    "confirmation_date", order == null ? "" : (ToDateTime(DT(order, "confirmation_date")).HasValue ? ToDateTime(DT(order, "confirmation_date")).Value.ToString("yyyy-MM-dd") : ""),
                    "packed_boxes", boxCountLookup.ContainsKey(I(row, "order_id")) ? boxCountLookup[I(row, "order_id")] : 0,
                    "balance_boxes", order == null ? 0 : D(order, "packing_balance_box_qty"),
                    "acted_at", FormatDateTime(DT(row, "acted_at")),
                    "acted_by", EmptyAs(S(row, "acted_by_name"), "-"),
                    "station_name", EmptyAs(S(row, "station_name"), "-"),
                    "action", HumanHistoryAction(statusLookup, row),
                    "remarks", S(row, "remarks")
                );
            }).ToList();
            var machineSelected = machineRows.Any(r => I(r, "order_id") == selectedOrderId)
                ? selectedOrderId
                : (machineRows.Count > 0 ? I(machineRows[0], "order_id") : 0);
            return Obj("rows", machineRows, "selected_order_id", machineSelected == 0 ? (object)null : machineSelected);
        }
        var rows = QueryAll(conn, "SELECT TOP 80 o.order_id, o.order_number, o.created_by, o.updated_at, o.workflow_stage_code, o.dispatch_status_code FROM tbl_orders AS o ORDER BY o.updated_at DESC, o.order_id DESC");
    ...
    private bool IsHistoryVisibleToUser(Dictionary<string, object> user, Dictionary<string, object> row, Dictionary<string, object> order)
    {
        ...
        if (role == "Machine User") return stationName == S(user, "station_name");
    ...
    private void ApplyProductionAction(OleDbConnection conn, Dictionary<string, object> user, Dictionary<string, object> order, Dictionary<string, object> station, Dictionary<string, object> queueEntry, string actionCode, string remarks, double? balanceBoxQty)
    {
        ...
        if (actionCode == "COMPLETED")
        {
            ...
            if (IsPackingStationName(stationName))
            {
                ...
                dispatchStatusCode = "PENDING_DISPATCH";
    ...
    private void CascadePackingToSubOrders(OleDbConnection conn, Dictionary<string, object> user, Dictionary<string, object> mainOrder, string remarks)
    {
        ...
        foreach (var subOrder in linkedSubOrders)
        {
            ...
            Execute(conn, "UPDATE tbl_orders SET workflow_stage_code = ?, dispatch_status_code = ?, updated_by = ?, updated_at = " + SqlDateLiteral(DateTime.Now) + ", last_action = ? WHERE order_id = ?",
                "DISPATCH_READY", "PENDING_DISPATCH", I(user, "user_id"), note, I(subOrder, "order_id"));
    ...
    private void MoveOrderToPlannerStation(OleDbConnection conn, Dictionary<string, object> user, Dictionary<string, object> order, Dictionary<string, object> targetStation)
    {
        ...
        var dispatchStatus = string.Equals(stationName, "Dispatch", StringComparison.OrdinalIgnoreCase) ? "PENDING_DISPATCH" : S(order, "dispatch_status_code");
    ...
    private int Execute(OleDbConnection conn, string sql, params object[] values)
    {
        using (var cmd = new OleDbCommand(sql, conn))
        {
            AddParameters(cmd, values);
    ...
    private static bool IsPackingStationName(string stationName)
    {
        return string.Equals(stationName, "Packing", StringComparison.OrdinalIgnoreCase)
            || string.Equals(stationName, "Packed", StringComparison.OrdinalIgnoreCase);
    }
