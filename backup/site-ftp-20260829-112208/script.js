(() => {
  ...
  const QR_ALLOWED_STATIONS = /* @__PURE__ */ new Set(["Hot Press", "Cutting", "Edgebanding", "Drilling", "Packing", "Packed"]);
  const API_ACTIONS = {
    "/api/session": "session",
    ...
    "/api/history-state": "history-state",
    "/api/dealers": "dealers",
    "/api/dealers/update": "dealers-update",
    "/api/dealers/customer-type": "dealers-customer-type",
  ...
  const refs = {
    ...
    dealerCustomerTypeChangeForm: document.querySelector("#dealer-customer-type-change-form"),
    dealerTypeChangeDealer: document.querySelector("#dealer-type-change-dealer"),
    dealerTypeChangeCustomerType: document.querySelector("#dealer-type-change-customer-type"),
    dealerTypeChangeHelper: document.querySelector("#dealer-type-change-helper"),
  ...
  function bindEvents() {
    ...
    (_h = refs.dealerCustomerTypeChangeForm) == null ? void 0 : _h.addEventListener("submit", onDealerCustomerTypeChangeSave);
    (_h = refs.dealerTypeChangeDealer) == null ? void 0 : _h.addEventListener("input", syncDealerCustomerTypeChangePreview);
    (_h = refs.dealerTypeChangeCustomerType) == null ? void 0 : _h.addEventListener("input", syncDealerCustomerTypeChangePreview);
  ...
  async function restoreSession() {
    ...
    try {
      ...
      if (result.authenticated) {
        ...
        if ((_b = (_a = state.session) == null ? void 0 : _a.sections) == null ? void 0 : _b.includes("history")) {
          await loadHistoryState(state.ui.selectedOrderId, false);
        }
  ...
  async function loadAppState(useHomeSection) {
    ...
    try {
      const deepState = needsDeepAppState();
      const deepSection = deepAppStateSection();
      const result = await apiGet("/api/app-state", true, {
        ...
        deep_state: deepState ? "1" : "",
        deep_section: deepSection
      });
      state.app = result;
  ...
  async function loadHistoryState(selectedOrderId, openModal) {
    if (!state.session) return;
    try {
      const response = await fetch(`history-cache.json?v=${Date.now()}`, { cache: "no-store" });
      if (!response.ok) {
        throw new Error("Unable to load history data.");
      }
      const raw = await response.text();
      const result = JSON.parse(String(raw || "").replace(/^\uFEFF/, ""));
      const visibleRows = filterHistoryRowsForSession(result.rows || []);
      const requestedOrderId = Number(selectedOrderId || state.ui.selectedOrderId || 0);
      const selectedRow = visibleRows.find((row) => Number(row.order_id) === requestedOrderId) || null;
      const resolvedSelectedOrderId = selectedRow ? requestedOrderId : visibleRows[0] && visibleRows[0].order_id || null;
      state.app = state.app || {};
      state.app.history = {
        rows: visibleRows,
        selected_order_id: resolvedSelectedOrderId
      };
      state.app.lifecycle = selectedRow ? {
        title: selectedRow.order_number || "Order History",
        summary: {
          order_number: selectedRow.order_number || "",
          workflow_stage: selectedRow.action || "",
          visible_stations: selectedRow.station_name || "-"
        },
        station_remarks: [],
        history: [
          {
            acted_at: selectedRow.acted_at || "",
            acted_by: selectedRow.acted_by || "-",
            station_name: selectedRow.station_name || "-",
            action: selectedRow.action || "Updated",
            remarks: selectedRow.remarks || ""
          }
        ]
      } : result.lifecycle || { title: "No Order Selected", summary: null, station_remarks: [], history: [] };
      state.ui.selectedOrderId = resolvedSelectedOrderId;
      if (openModal) state.ui.historyModalOpen = true;
      renderHistory();
      renderHistoryModal();
    } catch (error) {
      showMessage(error.message, "error");
  ...
  function filterHistoryRowsForSession(rows) {
    if (!state.session) return [];
    const role = String(state.session.role_name || "");
    const fullName = String(state.session.full_name || "");
    const stationName = String(state.session.station_name || "");
    return rows.filter((row) => {
      const rawAction = String(row.raw_action || "");
      const rowStation = String(row.station_name || "");
      if (role === "Admin" || role === "Management" || role === "Production Planner User") return true;
      if (role === "Data Entry" || role === "Quotation User") return Number(row.created_by || 0) === Number(state.session.user_id || 0);
      if (role === "Marketing User") return String(row.marketing_owner || "").toLowerCase() === fullName.toLowerCase();
      if (role === "Optimisation User") return rawAction === "OPTIMISATION_DONE";
      if (role === "Procurement User") return ["PO_PENDING", "PO_RAISED", "PARTIAL_MATERIAL_RECEIVED", "MATERIAL_RECEIVED", "CANCELLED"].includes(rawAction);
      if (role === "Machine User") return rowStation === stationName;
      if (role === "Dispatch User") return rowStation === "Dispatch" || ["PENDING_DISPATCH", "PARTIALLY_DISPATCHED", "DISPATCHED", "HOLD"].includes(rawAction);
      return false;
    });
  }
  ...
  function renderMasterLookups() {
    ...
    syncDealerCustomerTypeChangePreview();
  ...
  function needsDeepAppState() {
    return state.ui.historyModalOpen || state.ui.activeSection === "history" || state.ui.activeSection === "reports" || state.ui.activeSection === "email-log";
  }
  function deepAppStateSection() {
    if (state.ui.historyModalOpen) return "lifecycle";
    if (state.ui.activeSection === "history") return "history";
    if (state.ui.activeSection === "reports") return "reports";
    if (state.ui.activeSection === "email-log") return "email-log";
    return "";
  }
  ...
  function getSharedPlanningRows() {
    ...
    if (!rows.length) {
      rows = buildSharedPlanningFallbackRows();
    }
  ...
  function buildSharedPlanningFallbackRows() {
    var _a, _b;
    const recentOrders = ((_b = (_a = state.app) == null ? void 0 : _a.data_entry) == null ? void 0 : _b.recent_orders) || [];
    return recentOrders.map((row, index) => ({
      order_id: row.order_id,
      planning_rank: (index + 1) * 10,
      confirmation_date: "",
      order_number: row.order_number || "",
      ...
      current_stage_hint: row.last_action || row.workflow_stage || "",
      sla_date: "",
      edd: "",
      panel_qty: "",
      board_qty: "",
      urgency: "",
      priority: "",
      planner_remarks: row.last_action || "",
      assigned_station: row.visible_stations || "",
      partial_pending: false,
      planner_stage_key: "production",
      planner_stage_label: row.workflow_stage || "In Production"
    }));
  }
  ...
  function renderProductionQueueRow(row) {
    const showPackingBalance = row.current_station === "Packing" || row.current_station === "Packed";
  ...
  function renderProductionActionForm(rows) {
    ...
    refs.productionActionRemarks.placeholder = placeholderRow.current_station === "Packing" || placeholderRow.current_station === "Packed" ? "Packing: enter box qty" : "Enter remarks";
  ...
  function renderPackingBoxForm(rows) {
    ...
    const isPackingSession = ((_a = state.session) == null ? void 0 : _a.role_name) === "Machine User" && (((_b = state.session) == null ? void 0 : _b.station_name) === "Packing" || ((_b = state.session) == null ? void 0 : _b.station_name) === "Packed");
    const packingRows = (rows || []).filter((row) => row.current_station === "Packing" || row.current_station === "Packed");
    if (!isPackingSession) {
  ...
  async function onProductionActionFormClick(event) {
    ...
    const isPacking = selected.current_station === "Packing" || selected.current_station === "Packed";
    ...
    try {
      ...
      await apiPost("/api/production/action", {
        ...
        balance_box_qty: selected.current_station === "Packing" || selected.current_station === "Packed" ? selected.packing_balance_box_qty || 0 : ""
      });
    ...
    renderPagedTable({
      ...
      renderRow: (row) => `
        <tr>
          ...
          <td>${escapeHtml(row.planned_box_qty || "-")}</td>
  ...
  async function activateSection(sectionName) {
    ...
    if (state.session && sectionName === "history" && previousSection !== "history") {
      await loadHistoryState(state.ui.selectedOrderId, false);
      return;
    }
    if (state.session && previousSection !== sectionName && needsDeepAppState()) {
      await loadAppState(false);
    }
  }
  ...
  function selectedDealerForCustomerTypeChange() {
    if (!state.app || !refs.dealerTypeChangeDealer) return null;
    const dealerInput = refs.dealerTypeChangeDealer.value.trim();
    const dealerKey = dealerInput.toLowerCase();
    const dealers = state.app.data_entry.dealers || [];
    const exactName = dealers.find((item) => String(item.dealer_name || "").trim().toLowerCase() === dealerKey);
    if (exactName) return exactName;
    const exactCode = dealers.find((item) => String(item.dealer_code || "").trim().toLowerCase() === dealerKey);
    if (exactCode) return exactCode;
    const normalizedInput = normalizeSearchKey(dealerInput);
    const normalizedMatches = dealers.filter((item) =>
      normalizeSearchKey(item.dealer_name || "") === normalizedInput || normalizeSearchKey(item.dealer_code || "") === normalizedInput
    );
    return normalizedMatches.length === 1 ? normalizedMatches[0] : null;
  }
  function selectedCustomerTypeForChange() {
    if (!state.app || !refs.dealerTypeChangeCustomerType) return null;
    const customerType = refs.dealerTypeChangeCustomerType.value.trim().toLowerCase();
    return state.app.masters.customer_types.find((item) =>
      String(item.code || "").trim().toLowerCase() === customerType || String(item.name || "").trim().toLowerCase() === customerType
    ) || null;
  }
  function syncDealerCustomerTypeChangePreview() {
    if (!refs.dealerTypeChangeHelper) return;
    const dealer = selectedDealerForCustomerTypeChange();
    const customerType = selectedCustomerTypeForChange();
    if (!refs.dealerTypeChangeDealer || !refs.dealerTypeChangeDealer.value.trim()) {
      refs.dealerTypeChangeHelper.textContent = "Select a dealer to see current customer type.";
      return;
    }
    if (!dealer) {
      refs.dealerTypeChangeHelper.textContent = "Dealer not found or multiple similar dealers found. Select the exact dealer name from dropdown, or type the dealer code.";
      return;
    }
    const currentType = dealer.customer_type_code || "-";
    const nextType = customerType ? customerType.code || customerType.name : refs.dealerTypeChangeCustomerType.value.trim();
    refs.dealerTypeChangeHelper.textContent = `Matched: ${dealer.dealer_name || "-"} (${dealer.dealer_code || "no code"}) | Current customer type: ${currentType}${nextType ? ` | New customer type: ${nextType}` : ""}`;
  }
  ...
  async function onDealerCustomerTypeChangeSave(event) {
    event.preventDefault();
    const dealer = selectedDealerForCustomerTypeChange();
    const customerType = selectedCustomerTypeForChange();
    if (!dealer) {
      showMessage("Select a valid dealer from the dropdown.", "error");
      return;
    }
    if (!customerType) {
      showMessage("Select a valid customer type from the dropdown. Add it first in Customer Type Master if needed.", "error");
      return;
    }
    const currentType = String(dealer.customer_type_code || "").trim();
    const nextType = String(customerType.code || customerType.name || "").trim();
    if (!nextType) {
      showMessage("Customer type is required.", "error");
      return;
    }
    if (currentType.toLowerCase() === nextType.toLowerCase()) {
      showMessage("Dealer already has this customer type.", "success");
      return;
    }
    try {
      const result = await apiPost("/api/dealers/customer-type", {
        dealer_id: dealer.dealer_id,
        customer_type: nextType
      });
      dealer.customer_type_id = customerType.id || dealer.customer_type_id;
      dealer.customer_type_code = nextType;
      refs.dealerTypeChangeDealer.value = dealer.dealer_name || "";
      refs.dealerTypeChangeCustomerType.value = nextType;
      fillDatalist(refs.dealerOptions, state.app.data_entry.dealer_options);
      syncDealerCustomerTypeChangePreview();
      showMessage(`Customer type updated for ${dealer.dealer_name}. Orders updated: ${result.orders_updated || 0}.`, "success");
    } catch (error) {
      showMessage(error.message, "error");
    }
  }
  ...
  async function onProductionTableClick(event) {
    ...
    if (balanceSaveButton) {
      ...
      const stationName2 = balanceSaveButton.dataset.stationName || "Packed";
    ...
    const balanceBoxQty = stationName === "Packing" || stationName === "Packed" ? ((_b = document.querySelector(`#packing-balance-${orderId}`)) == null ? void 0 : _b.value) || "0" : "";
    try {
      await apiPost("/api/production/action", {
  ...
  async function onHistoryTableClick(event) {
    ...
    await openHistoryModalForOrder(Number(button.dataset.historyOrder));
  }
  ...
  async function openHistoryModalForOrder(orderId) {
    ...
    await loadHistoryState(orderId, true);
  }
