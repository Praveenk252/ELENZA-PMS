(() => {
  ...
  async function loadHistoryState(selectedOrderId, openModal) {
    ...
    try {
      const result = await apiGet("/api/history-state", true, {
        selected_order_id: selectedOrderId || state.ui.selectedOrderId || ""
      });
      state.app = state.app || {};
      state.app.history = {
        rows: result.rows || [],
        selected_order_id: result.selected_order_id || null
      };
      state.app.lifecycle = result.lifecycle || { title: "No Order Selected", summary: null, station_remarks: [], history: [] };
      state.ui.selectedOrderId = result.selected_order_id || state.ui.selectedOrderId || null;
