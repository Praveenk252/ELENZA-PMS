...
public class PmsApiHandler : IHttpHandler, IRequiresSessionState
{
    ...
    private void HandleProductionBalanceSave(HttpContext context)
    {
        using (var conn = OpenConnection(context))
        {
            ...
            var orderId = IntRequired(Value(context, "order_id"), "Order is required.");
            var stationName = Require(Value(context, "station_name"), "Station is required.");
            if (!string.Equals(stationName, "Packing", StringComparison.OrdinalIgnoreCase))
    ...
    private Dictionary<string, object> BuildAppState(OleDbConnection conn, Dictionary<string, object> user, Dictionary<string, string> filters)
    {
        ...
        var audits = deepState && sections.Contains("reports") ? LoadAudits(conn, user, orders) : new List<Dictionary<string, object>>();
        ...
        var planningState = sections.Contains("planner") ? BuildPlanningState(conn, user, orders, statusLookup) : Obj("rows", new List<object>(), "can_edit", false);
        ...
        var reportsState = deepState && sections.Contains("reports") ? BuildReportsState(conn, orders, audits, statusLookup, filters) : EmptyReportsState();
        var historyState = deepState && sections.Contains("history") ? BuildHistoryState(conn, user, orders, statusLookup, ToInt(filters["selected_order_id"])) : EmptyHistoryState();
        var emailLogState = deepState && sections.Contains("email-log") ? GetMailStatus(conn) : Obj("rows", new List<object>());
        ...
        var lifecycle = deepState && (sections.Contains("reports") || sections.Contains("history"))
    ...
    private string PlanningStageKey(Dictionary<string, object> order)
    {
        ...
        if (string.Equals(highestReachedStation, "Dispatch", StringComparison.OrdinalIgnoreCase) || string.Equals(highestReachedStation, "Packing", StringComparison.OrdinalIgnoreCase)) return "packed";
    ...
    private string LatestPlannerStatusLabel(Dictionary<string, object> order, Dictionary<string, Dictionary<string, string>> statusLookup)
    {
        ...
        if (string.Equals(highestReachedStation, "Packing", StringComparison.OrdinalIgnoreCase)) return "Packed";
        if (string.Equals(highestReachedStation, "QC", StringComparison.OrdinalIgnoreCase)) return "QC";
    ...
    {
        var rows = QueryAll(conn, "SELECT TOP 400 h.*, o.order_number, u.full_name AS acted_by_name, s.machine_name AS station_name FROM ((tbl_order_history AS h INNER JOIN tbl_orders AS o ON h.order_id = o.order_id) LEFT JOIN tbl_users AS u ON h.acted_by = u.user_id) LEFT JOIN tbl_machines AS s ON h.station_id = s.machine_id ORDER BY h.acted_at DESC, h.history_id DESC");
        ...
        foreach (var row in rows)
        {
            ...
            filtered.Add(Obj("order_id", I(row, "order_id"), "order_number", S(row, "order_number"), "acted_at", FormatDateTime(DT(row, "acted_at")), "acted_by", EmptyAs(S(row, "acted_by_name"), "-"), "station_name", EmptyAs(S(row, "station_name"), "-"), "action", HumanHistoryAction(statusLookup, row), "remarks", S(row, "remarks")));
        }
    ...
    private bool IsHistoryVisibleToUser(Dictionary<string, object> user, Dictionary<string, object> row, Dictionary<string, object> order)
    {
        ...
        if (role == "Optimisation User") return S(row, "action_code") == "OPTIMISATION_DONE";
        if (role == "Procurement User") return ProcurementStatusCodes.Contains(S(row, "action_code"));
        if (role == "Production Planner User") return true;
        if (role == "Machine User") return S(row, "station_name") == S(user, "station_name");
        if (role == "Dispatch User") return S(row, "station_name") == "Dispatch" || DispatchStatusCodes.Contains(S(row, "action_code"));
        return false;
    }
    ...
    private void CascadePackingToSubOrders(OleDbConnection conn, Dictionary<string, object> user, Dictionary<string, object> mainOrder, string remarks)
    {
        ...
        foreach (var subOrder in linkedSubOrders)
        {
            ...
            var packingStation = sequenceStations.FirstOrDefault(m => string.Equals(S(m, "machine_name"), "Packing", StringComparison.OrdinalIgnoreCase));
    ...
    {
        var dbPath = Path.Combine(siteRoot, "App_Data", "LSP.accdb");
        var providers = new[]
        {
